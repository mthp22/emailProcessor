...
class BestmedPayloadProcessor:
    ...
    def process_email_json_payload(cls, email_file_name):
        ...
        try:
            ...
            event_id = email_record.get('event_id')
            ...
            if not json_payload:
                ...
                AuditLogger.log_event(
                    ...
                    details='Email exists but has no JSON payload'
                )
            ...
            AuditLogger.log_event(
                ...
                email_file_name=email_file_name,
                event_id=event_id,
                operation='process_email_json_payload_started',
                status='info',
                details='Started processing JSON payload'
            )
            
            zip_data, file_count = cls.create_zip_from_email_attachments(email_file_name)

            if not zip_data or file_count == 0:
                log(f"[INFO] No attachments found for email: {email_file_name} - marking as N/A")
                AuditLogger.log_event(
                    event_type='json_payload_processing',
                    email_file_name=email_file_name,
                    event_id=event_id,
                    operation='no_attachments_found',
                    status='warning',
                    details=f'No attachments found for email. Marking as N/A',
                    data_extracted={'attachment_count': 0}
                )
            ...
            if not file_binary:
                ...
                AuditLogger.log_event(
                    ...
                    email_file_name=email_file_name,
                    event_id=event_id,
                    operation='encode_zip_failed',
                    status='failed',
                    error_message='Failed to encode ZIP file to base64',
                    data_extracted={'file_count': file_count}
                )
                supabase_write.table('emails').update({
                    'payload_status': 'failed'
                }).eq('id', email_record['id']).execute()
            ...
            # Update JSON payload with file binary
            email_base_name = email_file_name.replace('.msg', '').replace('.eml', '')
            ...
            if update_result.data:
                ...
                AuditLogger.log_event(
                    ...
                    email_file_name=email_file_name,
                    event_id=event_id,
                    operation='process_email_json_payload_completed',
                    status='success',
                    details=f'Successfully processed JSON payload with {file_count} attachments',
                    data_extracted={
                        'file_count': file_count,
                        'zip_file_name': f"{email_base_name}_attachments.zip",
                        'payload_status': 'validated'
                    }
                )
                return True
                ...
                AuditLogger.log_event(
                    ...
                    email_file_name=email_file_name,
                    event_id=event_id,
                    operation='database_update_failed',
                    status='failed',
                    error_message='Failed to update database with new payload',
                    data_extracted={'file_count': file_count}
                )
                supabase_write.table('emails').update({
                    'payload_status': 'failed'
            ...
            AuditLogger.log_event(
                ...
                email_file_name=email_file_name,
                event_id=event_id,
                operation='process_email_json_payload_exception',
                status='failed',
                error_message=str(e),
    ...
    def process_all_emails_with_json_payloads(cls):
        ...
        try:
            AuditLogger.log_event(
                ...
                status= 'info',
                details='Started batch processing of emails with JSON payloads'
            )
            ...
            if not result.data:
                ...
                status='info',
                details='No emails with JSON payloads found'
                )
            ...
            for email_record in result.data:
                ...
                if success:
                    ...
                    log(f"[SUCCESS] Processed {email_file_name}")
                     ...
                     # Check if it was marked as N/A (no attachments)
                    check_result = supabase_read.table('emails').select('payload_status').eq('file_name', email_file_name).execute()
                    if check_result.data and check_result.data[0].get('payload_status') == 'N/A':
                        no_attachments_count += 1
                        log(f"[N/A] No attachments for {email_file_name}")
                    else:
                        error_count += 1
            ...
            AuditLogger.log_event(
                ...
                operation='batch_processing_completed',
                status='success',
                details=f'Batch processing complete: {processed_count} processed, {no_attachments_count} without attachments, {error_count} errors, {skipped_count} skipped',
                data_extracted={
                    'processed': processed_count,
                    'no_attachments': no_attachments_count,
                    'errors': error_count,
                    'skipped': skipped_count
                }
            )    
            log(f"[COMPLETE] Processed {processed_count} emails, {error_count} errors")
            return {"processed": processed_count, "errors": error_count, "skipped": skipped_count, "no_attachments": no_attachments_count}
            ...
            return {"processed": 0, "errors": 1, "skipped": 0, "no_attachments": 0}
...
# Auditing class starts here
class AuditLogger:
    @classmethod
    def log_event(cls, event_type, email_file_name=None, event_id=None, 
                  operation=None, status='success', details=None, error_message=None,
                  user_action=None, data_extracted=None, classification=None):
        """
        Log an audit event to the database
        
        Args:
            event_type: Type of event (e.g., 'email_processing', 'json_payload', 'validation', 'user_action')
            email_file_name: Email file being processed
            event_id: Associated event ID from emails table
            operation: Specific operation being performed
            status: 'success', 'failed', 'warning', 'info'
            details: Additional details about the event
            error_message: Error message if status is 'failed'
            user_action: User action that triggered the event (e.g., 'validate_clicked', 'view_payload')
            data_extracted: JSON data extracted during operation
        ...
        try:
            ...
            result = supabase_write.table('audit_logs').insert(audit_entry).execute()
            
            if result.data:
                log(f"[AUDIT] Logged {event_type} event for {email_file_name or 'system'} - Status: {status}")
                return True
            else:
                log(f"[AUDIT ERROR] Failed to log event: {audit_entry}")
                return False
                
        except Exception as e:
            log(f"[AUDIT ERROR] Exception logging event: {e}")
            import traceback
            traceback.print_exc()
            return False
    
    @classmethod
    def get_audit_logs(cls, limit=100, event_type=None, status=None, email_file_name=None, 
        ...
        Args:
            ...
            email_file_name: Filter by email file name
            start_date: Filter by start date
            end_date: Filter by end date
        """
        try:
            query = supabase_read.table('audit_logs').select('*')
            ...
            if email_file_name:
                query = query.ilike('email_file_name', f'%{email_file_name}%')
            if start_date:
            ...
            log(f"[AUDIT ERROR] Failed to retrieve audit logs: {e}")
            return []
    
    @classmethod
    def get_event_timeline(cls, event_id):
        """Get complete timeline for a specific event ID"""
        try:
            result = supabase_read.table('audit_logs').select('*').eq(
                'event_id', event_id
            ...
            log(f"[AUDIT ERROR] Failed to retrieve event timeline: {e}")
            return []
    ...
    def get_audit_statistics(cls):
        ...
        try:
            all_logs = supabase_read.table('audit_logs').select('status, event_type').execute()
            ...
            log(f"[AUDIT ERROR] Failed to get statistics: {e}")
            return {
                'total_events': 0,
                'success_count': 0,
                'failed_count': 0,
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .timeline-item.success::before { background: #4ec9b0; }
                .timeline-item.failed::before { background: #f48771; }
                .timeline-item.warning::before { background: #dcdcaa; }
                ...
                .status-badge.success { background: #2d5016; color: #4ec9b0; }
                .status-badge.failed { background: #3d1f1f; color: #f48771; }
                .status-badge.warning { background: #3d3d1f; color: #dcdcaa; }
                .status-badge.info { background: #1f2d3d; color: #569cd6; }
                ...
                .analytics-header:hover { background: #333; }
                .analytics-title {
                    font-size: 16px;
                    font-weight: bold;
                    color: #4ec9b0;
                ...
                .analytics-content.expanded { display: block; }
                ...
                .kpi-bar.success { background: #4ec9b0; color: black; }
                .kpi-bar.failed { background: #f14c4c; color: white; }
                .kpi-bar.partial { background: #cca700; color: black; }
                .kpi-bar.pending { background: #cca700; color: black; }
                .kpi-bar.validated { background: #4ec9b0; color: black; }
                ...
                .classification-name { color: #d4d4d4; }
        ...
        <body>
            ...
            <div class="analytics-section">
                <div class="analytics-header" onclick="toggleAnalytics()">
                    <span class="analytics-title">📊 Analytics & KPIs</span>
                    <span class="analytics-toggle" id="analytics-toggle">▼ Expand</span>
                </div>
                <div class="analytics-content" id="analytics-content">
                    <!-- Status Distributions -->
                    <div class="kpi-grid">
                        <div class="kpi-card">
                            <div class="kpi-card-title">Processing Status Distribution</div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Success</span>
                                    <span id="kpi-processing-success-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar success" id="kpi-processing-success" style="width: 0%"></div>
                                </div>
                            </div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Failed</span>
                                    <span id="kpi-processing-failed-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar failed" id="kpi-processing-failed" style="width: 0%"></div>
                                </div>
                            </div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Partial</span>
                                    <span id="kpi-processing-partial-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar partial" id="kpi-processing-partial" style="width: 0%"></div>
                                </div>
                            </div>
                        </div>

                        <div class="kpi-card">
                            <div class="kpi-card-title">Payload Status Distribution</div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Validated</span>
                                    <span id="kpi-payload-validated-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar validated" id="kpi-payload-validated" style="width: 0%"></div>
                                </div>
                            </div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Pending</span>
                                    <span id="kpi-payload-pending-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar pending" id="kpi-payload-pending" style="width: 0%"></div>
                                </div>
                            </div>
                            <div class="kpi-item">
                                <div class="kpi-label">
                                    <span>Failed</span>
                                    <span id="kpi-payload-failed-text">0 (0%)</span>
                                </div>
                                <div class="kpi-bar-container">
                                    <div class="kpi-bar failed" id="kpi-payload-failed" style="width: 0%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="kpi-grid">
                        <div class="kpi-card">
                            <div class="kpi-card-title">Processing Performance</div>
                            <div class="kpi-metrics">
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-avg-duration">0</div>
                                    <div class="kpi-metric-label">Avg (ms)</div>
                                </div>
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-min-duration">0</div>
                                    <div class="kpi-metric-label">Min (ms)</div>
                                </div>
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-max-duration">0</div>
                                    <div class="kpi-metric-label">Max (ms)</div>
                                </div>
                            </div>
                        </div>

                        <div class="kpi-card">
                            <div class="kpi-card-title">Success Rates</div>
                            <div class="kpi-metrics">
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-processing-rate">0%</div>
                                    <div class="kpi-metric-label">Processing</div>
                                </div>
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-validation-rate">0%</div>
                                    <div class="kpi-metric-label">Validation</div>
                                </div>
                            </div>
                        </div>

                        <div class="kpi-card">
                            <div class="kpi-card-title">Confidence Scores</div>
                            <div class="kpi-metrics">
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-avg-confidence">0</div>
                                    <div class="kpi-metric-label">Average</div>
                                </div>
                                <div class="kpi-metric">
                                    <div class="kpi-metric-value" id="kpi-confidence-count">0</div>
                                    <div class="kpi-metric-label">Classified</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="kpi-grid">
                        <div class="kpi-card">
                            <div class="kpi-card-title">Document Classification Breakdown</div>
                            <div class="classification-list" id="classification-list">
                                <div style="text-align: center; color: #666; padding: 20px;">No classification data available</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="control-panel">
                ...
                <button class="process-btn" onclick="processAllPayloads()"> Complete JSON Payloads</button>
                <button class="download-btn" onclick="downloadCompletePayloads()"> Download All JSON Payloads</button>
            ...
            <div class="tabs">
                <button class="tab active" onclick="showTab('processing', this)">📧 Email Processing Status</button>
                <button class="tab" onclick="showTab('events', this)">🔖 Event IDs</button>
                <button class="tab" onclick="showTab('json-completion', this)">📄 JSON Payload Completion</button>
                <button class="tab" onclick="switchTab('audit', this)">📋 Audit and Logging</button>
            </div>
            ...
            <div id="audit-tab" class="tab-content">
                        ...
                        <div class="audit-stats">
                            ...
                            <div class="stat-card success">
                                <h3>Successful</h3>
                                <div class="value" id="success-events">0</div>
                            </div>
                            <div class="stat-card failed">
                                <h3>Failed</h3>
                                <div class="value" id="failed-events">0</div>
                            </div>
                            <div class="stat-card warning">
                                <h3>Warnings</h3>
                                <div class="value" id="warning-events">0</div>
                            </div>
                        </div>
                        ...
                        <div class="audit-filters">
                            <div>
                                ...
                                <select id="filter-event-type" onchange="loadAuditLogs()">
                                    <option value="">All</option>
                                    <option value="email_processing">Email Processing</option>
                                    <option value="json_payload_processing">JSON Payload</option>
                            ...
                            <div>
                                <label>Status:</label>
                                <select id="filter-status" onchange="loadAuditLogs()">
                                    <option value="">All</option>
                            ...
                            <div>
                                ...
                                <input type="text" id="filter-email" placeholder="Email file name..." onkeyup="loadAuditLogs()">
                            </div>
                            
                            <div>
                                <label>Limit:</label>
                                <select id="filter-limit" onchange="loadAuditLogs()">
            ...
            <script>
                ...
                async function loadAuditStatistics() {
                    try {
                        const response = await fetch('/audit-statistics');
                        const stats = await response.json();
                        
                        document.getElementById('total-events').textContent = stats.total_events || 0;
                        document.getElementById('success-events').textContent = stats.success_count || 0;
                        document.getElementById('failed-events').textContent = stats.failed_count || 0;
                        document.getElementById('warning-events').textContent = stats.warning_count || 0;
                    } catch (error) {
                ...
                async function loadAuditLogs() {
                    try {
                        ...
                        if (email) url += `&email_file_name=${email}`;
                ...
                function renderAuditLogs(logs) {
                    ...
                    if (logs.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="8" style="text-align: center;">No audit logs found</td></tr>';
                        return;
                    }
                    ...
                    logs.forEach(log => {
                        ...
                        tr.innerHTML = `
                            <td>${timestamp}</td>
                            <td><span class="event-type-badge">${log.event_type || 'N/A'}</span></td>
                            <td>${log.email_file_name || 'N/A'}</td>
                            <td><code>${log.event_id || 'N/A'}</code></td>
                            <td>${log.operation || 'N/A'}</td>
                            <td><span class="status-badge ${statusClass}">${log.status || 'N/A'}</span></td>
                            <td>${truncate(log.details || (log.error_message ? 'Error: ' + log.error_message : 'N/A'), 50)}</td>
                            <td>
                                ${log.event_id ? `<button class="view-timeline-btn" onclick="viewTimeline('${log.event_id}')">📅 Timeline</button>` : ''}
                            </td>
                        `;
                ...
                async function viewTimeline(eventId) {
                    try {
                        const response = await fetch(`/event-timeline/${eventId}`);
                        ...
                        if (timeline.length === 0) {
                            content.innerHTML = '<p>No timeline events found for this Event ID.</p>';
                        } else {
                            content.innerHTML = '<div class="timeline-view">' + 
                                timeline.map(event => `
                                    <div class="timeline-item ${event.status}">
                                        <div class="timeline-header">
                                            <span class="timeline-operation">${event.operation || 'N/A'}</span>
                                            <span class="timeline-timestamp">${new Date(event.timestamp).toLocaleString()}</span>
                                        </div>
                                        <div class="timeline-details">
                                            <strong>Status:</strong> <span class="status-badge ${event.status}">${event.status}</span><br>
                                            ${event.details ? `<strong>Details:</strong> ${event.details}<br>` : ''}
                                            ${event.data_extracted ? `<strong>Data:</strong> <code>${JSON.stringify(event.data_extracted)}</code><br>` : ''}
                                            ${event.error_message ? `<div class="timeline-error">${event.error_message}</div>` : ''}
                                        </div>
                                    </div>
                                `).join('') +
                                '</div>';
                ...
                setInterval(() => {
                    const auditTab = document.getElementById('audit-tab');
                    if (auditTab.style.display === 'block') {
                        loadAuditStatistics();
                        loadAuditLogs();
                    }
                }, 30000);
                ...
                function showTab(tabName, element) {
                    ...
                    document.querySelectorAll('.tab-content').forEach(tab => {
                        ...
                        tab.style.display = 'none'; // Clear inline styles
                    });
                    ...
                    selectedTab.style.display = 'block'; // Set inline style
                ...
                async function loadAllData() {
                    await Promise.all([loadProcessingData(), loadEventsData(), loadCompletionData(), loadKPIData()]);
                }
                ...
                async function loadKPIData() {
                    try {
                        ...
                        if (data.error) {
                            console.error('Error loading KPI data:', data.error);
                            return;
                        }
                ...
                function updateKPIDisplay(kpi) {
                    ...
                    const processingStatus = kpi.processing_status;
                    document.getElementById('kpi-processing-success').style.width = processingStatus.success.percentage + '%';
                    document.getElementById('kpi-processing-success-text').textContent =
                        `${processingStatus.success.count} (${processingStatus.success.percentage}%)`;

                    document.getElementById('kpi-processing-failed').style.width = processingStatus.failed.percentage + '%';
                    document.getElementById('kpi-processing-failed-text').textContent =
                        `${processingStatus.failed.count} (${processingStatus.failed.percentage}%)`;

                    document.getElementById('kpi-processing-partial').style.width = processingStatus.partial.percentage + '%';
                    document.getElementById('kpi-processing-partial-text').textContent =
                        `${processingStatus.partial.count} (${processingStatus.partial.percentage}%)`;

                    // Payload Status Distribution
                    const payloadStatus = kpi.payload_status;
                    document.getElementById('kpi-payload-validated').style.width = payloadStatus.validated.percentage + '%';
                    document.getElementById('kpi-payload-validated-text').textContent =
                        `${payloadStatus.validated.count} (${payloadStatus.validated.percentage}%)`;

                    document.getElementById('kpi-payload-pending').style.width = payloadStatus.pending.percentage + '%';
                    document.getElementById('kpi-payload-pending-text').textContent =
                        `${payloadStatus.pending.count} (${payloadStatus.pending.percentage}%)`;

                    document.getElementById('kpi-payload-failed').style.width = payloadStatus.failed.percentage + '%';
                    document.getElementById('kpi-payload-failed-text').textContent =
                        `${payloadStatus.failed.count} (${payloadStatus.failed.percentage}%)`;

                    // Processing Performance
                    document.getElementById('kpi-avg-duration').textContent = kpi.processing_performance.avg_duration_ms;
                    document.getElementById('kpi-min-duration').textContent = kpi.processing_performance.min_duration_ms;
                    document.getElementById('kpi-max-duration').textContent = kpi.processing_performance.max_duration_ms;

                    // Success Rates
                    document.getElementById('kpi-processing-rate').textContent = kpi.success_rates.processing + '%';
                    document.getElementById('kpi-validation-rate').textContent = kpi.success_rates.payload_validation + '%';

                    // Confidence Scores
                    document.getElementById('kpi-avg-confidence').textContent = kpi.confidence_scores.average;
                    document.getElementById('kpi-confidence-count').textContent = kpi.confidence_scores.count;

                    // Classification Breakdown
                    const classificationList = document.getElementById('classification-list');
                    const classifications = kpi.classification.distribution;

                    if (Object.keys(classifications).length === 0) {
                        classificationList.innerHTML = '<div style="text-align: center; color: #666; padding: 20px;">No classification data available</div>';
                    } else {
                        ...
                        classificationList.innerHTML = sortedClassifications.map(([name, count]) => `
                            <div class="classification-item">
                                <span class="classification-name">${name || 'Unclassified'}</span>
...
def audit_logs():
    ...
    try:
        ...
        email_file_name = request.args.get('email_file_name')
...
@app.route("/event-timeline/<event_id>")
def event_timeline(event_id):
    """Get complete timeline for an event"""
    try:
        timeline = AuditLogger.get_event_timeline(event_id)
        return jsonify(timeline)
        
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/test-audit-log")
def test_audit_log():
    ...
    try:
        ...
        AuditLogger.log_event(
            ...
            email_file_name='test_email.msg',
            event_id='TEST_EVENT_123',
            operation='test_audit_logging',
            status='success',
            details='Testing audit logging system',
            data_extracted={'test': 'data'}
        )
        
        # Retrieve the log to confirm it was saved
        logs = AuditLogger.get_audit_logs(limit=1, event_type='system_test')
        ...
        if logs:
            ...
            return jsonify({
                "status": "warning",
                "message": "Log was created but not retrieved. Check database."
            })
...
def get_kpi():
    ...
    try:
        # 1. Get all emails data for processing
        emails_result = supabase_read.table('emails').select(
            'id, processing_status, payload_status, processing_duration_ms, final_classification, confidence_score, attachment_count'
        ).execute()

        emails = emails_result.data
        total_emails = len(emails)

        # 2. Get all attachments count
        attachments_result = supabase_read.table('attachments').select('id', count='exact').execute()
        total_attachments = attachments_result.count if hasattr(attachments_result, 'count') else 0

        # Initialize counters
        processing_status_counts = {'success': 0, 'failed': 0, 'partial': 0}
        payload_status_counts = {'pending': 0, 'validated': 0, 'failed': 0}
        ...
        for email in emails:
            # Processing status
            status = email.get('processing_status', 'success')
            processing_status_counts[status] = processing_status_counts.get(status, 0) + 1

            # Payload status
            payload_status = email.get('payload_status', 'pending')
            payload_status_counts[payload_status] = payload_status_counts.get(payload_status, 0) + 1

            # Processing duration
            if email.get('processing_duration_ms'):
                try:
                    processing_durations.append(float(email['processing_duration_ms']))
                except (ValueError, TypeError):
                    pass

            # Classification
            classification = email.get('final_classification')
            if classification:
            ...
            if email.get('confidence_score') is not None:
                try:
                    confidence_scores.append(float(email['confidence_score']))
                except (ValueError, TypeError):
                    pass

            # Attachment count
            if email.get('attachment_count') is not None:
                try:
                    attachment_counts.append(int(email['attachment_count']))
                except (ValueError, TypeError):
                    pass

        # Calculate metrics
        avg_processing_duration = sum(processing_durations) / len(processing_durations) if processing_durations else 0
        min_processing_duration = min(processing_durations) if processing_durations else 0
        max_processing_duration = max(processing_durations) if processing_durations else 0

        avg_confidence_score = sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0

        avg_attachments_per_email = sum(attachment_counts) / len(attachment_counts) if attachment_counts else 0

        # Calculate success rates
        processing_success_rate = (processing_status_counts['success'] / total_emails * 100) if total_emails > 0 else 0
        payload_validation_rate = (payload_status_counts['validated'] / total_emails * 100) if total_emails > 0 else 0

        # Format response
        kpi_data = {
            "processing_status": {
                "success": {
                    "count": processing_status_counts['success'],
                    "percentage": round(processing_status_counts['success'] / total_emails * 100, 2) if total_emails > 0 else 0
                },
                "failed": {
                    "count": processing_status_counts['failed'],
                    "percentage": round(processing_status_counts['failed'] / total_emails * 100, 2) if total_emails > 0 else 0
                },
                "partial": {
                    "count": processing_status_counts['partial'],
                    "percentage": round(processing_status_counts['partial'] / total_emails * 100, 2) if total_emails > 0 else 0
                }
            },
            "payload_status": {
                "pending": {
                    "count": payload_status_counts['pending'],
                    "percentage": round(payload_status_counts['pending'] / total_emails * 100, 2) if total_emails > 0 else 0
                },
                "validated": {
                    "count": payload_status_counts['validated'],
                    "percentage": round(payload_status_counts['validated'] / total_emails * 100, 2) if total_emails > 0 else 0
                },
                "failed": {
                    "count": payload_status_counts['failed'],
                    "percentage": round(payload_status_counts['failed'] / total_emails * 100, 2) if total_emails > 0 else 0
                }
            },
            "processing_performance": {
                "avg_duration_ms": round(avg_processing_duration, 2),
                "min_duration_ms": round(min_processing_duration, 2),
                "max_duration_ms": round(max_processing_duration, 2)
            },
            "success_rates": {
                "processing": round(processing_success_rate, 2),
                "payload_validation": round(payload_validation_rate, 2)
            },
            "classification": {
                "distribution": classification_counts,
                "total_classified": len(confidence_scores)
            },
            "confidence_scores": {
                "average": round(avg_confidence_score, 2),
                "count": len(confidence_scores)
            },
            "totals": {
                "emails": total_emails,
                "attachments": total_attachments,
                "avg_attachments_per_email": round(avg_attachments_per_email, 2)
            }
        }
...
def download_complete_json_payloads():
    """Download all JSON payloads (complete and incomplete) as ZIP"""
    try:
        import zipfile
        from io import BytesIO
        
        # Get all emails with JSON payloads (complete or not)
        result = supabase_read.table('emails').select(
            'file_name, json_payload, payload_status'
        ).not_.is_('json_payload', 'null').execute()
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            # Create folders for complete and incomplete
            for email in result.data:
                json_payload = email['json_payload']
                if isinstance(json_payload, str):
                    json_payload = json.loads(json_payload)
                
                # Check if payload has file binary
                file_binary = json_payload.get('fileBinary', '')
                has_binary = file_binary and len(file_binary) > 100
                payload_status = email.get('payload_status', 'pending')
                
                # Create filename
                email_name = email['file_name'].replace('.msg', '').replace('.eml', '')
                
                # Format the payload
                formatted_payload = format_bestmed_payload(json_payload)
                
                # Determine folder based on completion status
                if has_binary and payload_status == 'validated':
                    folder = "complete_payloads"
                    complete_count += 1
                elif payload_status == 'N/A':
                    folder = "no_attachments"
                    incomplete_count += 1
                else:
                    folder = "incomplete_payloads"
                    incomplete_count += 1
                
                json_filename = f"{folder}/{email_name}_payload.json"
                
                # Add to ZIP
                zf.writestr(json_filename, json.dumps(formatted_payload, indent=2))
        ...
        log(f"[DOWNLOAD] Generated ZIP with {complete_count} complete and {incomplete_count} incomplete payloads")
        
        return send_file(
            memory_file,
            as_attachment=True,
            download_name=f'bestmed_all_payloads_{timestamp}.zip',
            mimetype='application/zip'
        )
        
    except Exception as e:
        log(f"[ERROR] Failed to generate ZIP: {e}")
        return jsonify({"error": str(e)}), 500
